...
"""
import os, sys, re, json, time, base64, unicodedata, argparse, hashlib, socket
...
def emit_empty(want_json, msg):
    ...
    if ai_diagnostic_code():
        print("AI_DIAGNOSTIC:" + ai_diagnostic_code())
    print("・" + msg + "（空プールを返します）")
    if want_json:
        print("\n---- JSON ----")
        print("[]")
...
# AI失敗時に、秘密情報を含めず原因だけを親処理へ渡す固定コード。
# APIキー・URL・エラー本文・検索語はここへ出さない。
_AI_DIAGNOSTIC_CODES = frozenset({
    "config_missing", "budget_exhausted", "http_429", "http_5xx",
    "http_error", "network_error", "timeout", "parse_error", "empty_response",
})
_AI_DIAGNOSTIC = ""


def set_ai_diagnostic(code):
    """許可した固定コードだけを診断結果として保持する。"""
    global _AI_DIAGNOSTIC
    if code in _AI_DIAGNOSTIC_CODES:
        _AI_DIAGNOSTIC = code


def ai_diagnostic_code():
...
def load_gemini_conf():
    if not os.path.exists(GEMINI_CONF):
        set_ai_diagnostic("config_missing")
        return None
    try:
...
_HUMAN_ONLY_COOL_RE = re.compile(r"人用|大人用|子供用|こども用|ベビー用|人間用|大人|子供|こども|ベビー", re.I)
...
def apply_intent_category_evidence(items, theme="", angle_title=""):
    ...
    for row in items or []:
        ...
        human_only = (
            bool(_HUMAN_ONLY_COOL_RE.search(text)) and not has_pet
        ) or (
            bool(_HUMAN_ONLY_COOL_RE.search(title)) and not title_has_pet
        )
        if human_only:
            row["ai_score"] = 0
...
def gemini_rerank(conf, intent_text, intent_key, pool, few_shot=None):
    ...
    if not ok:
        set_ai_diagnostic("budget_exhausted")
    ...
    for attempt in range(1, 4):                     # 503/タイムアウトは一時的なので最大3回
        try:
            ...
            set_ai_diagnostic("http_5xx" if e.code in (500, 503) else "http_error")
            print("    Gemini HTTP %s: %s" % (e.code, body_txt)); return pool, False
        except Exception as e:
            if attempt < 3:
                print("    Gemini %s（%d回目）…%d秒待って再試行" % (type(e).__name__, attempt, 6 * attempt))
                time.sleep(6 * attempt); continue
            set_ai_diagnostic("timeout" if isinstance(e, (TimeoutError, socket.timeout)) else "network_error")
            print("    Gemini 失敗: %s: %s" % (type(e).__name__, e)); return pool, False
    if data is None:
        set_ai_diagnostic("empty_response")
        return pool, False
